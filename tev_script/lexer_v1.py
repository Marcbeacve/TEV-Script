...
class LexerV1:
        ...
        while self.index < len(self.text):
            ...
            if char.isalpha() or char.isdigit():
                raise TevScriptError(
                    "TEVS_V1_LEX_IDENTIFIER_NON_ASCII",
                    f"identifier characters must be ASCII, got {char!r}",
                    self._span(start),
                )
            raise TevScriptError(
                "TEVS_V1_LEX_UNKNOWN_CHARACTER",
    ...
    def _token(self, kind: str, text: str, start: tuple[int, int, int]) -> Token:
    ...
    def _string(self, start: tuple[int, int, int]) -> Token:
        ...
        while self.index < len(self.text):
            ...
            self._advance(char)
        raise TevScriptError(
            "TEVS_V1_LEX_STRING_UNTERMINATED",
