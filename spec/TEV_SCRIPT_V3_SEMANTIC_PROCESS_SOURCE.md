...
Blank lines and lines beginning with `#` are nonsemantic. Whitespace between tokens is nonsemantic. Fact/transform/label declaration order is nonsemantic except that label order does not determine control flow; labels are resolved symbolically and the compiler emits a canonical label order by lexical label name. `entry` names the entry label.
...
transformations sorted by transform id
...
`source_semantic_hash` is SHA-256 of that canonical model and is passed to IR V5.

## Compatibility

The V3 CLI may also dispatch unchanged V2 source to the existing V2 compiler/runtime, but V2 source remains language version `2.0.0` and keeps Program IR V4 identity. The V3 semantic-process parser never rewrites a V2 program into V3 semantics.
