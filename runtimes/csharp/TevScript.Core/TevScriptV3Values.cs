...
public static class TevScriptValueCodecV3
{
    ...
    public static TevScriptValueV3 Decode(string typeId, JsonElement raw, TevScriptTypeTableV3 table, string context = "value", int depth = 1)
    {
        ...
        if (descriptor.Kind == TevScriptTypeKindV3.Record)
        {
            ...
            RequireValueKeys(record, context + ".$record", typeId, "record encoded type mismatch", "type", "fields");
            ...
            for (var index = 0; index < descriptor.Fields.Count; ++index)
            {
                ...
                RequireValueKeys(field, $"{context}.$record.fields[{index}]", expected.TypeId, "record field shape mismatch", "name", "value");
        ...
        if (descriptor.Kind == TevScriptTypeKindV3.Enum)
        {
            ...
            RequireValueKeys(value, context + ".$enum", typeId, "enum encoded type mismatch", "type", "variant");
        ...
        if (descriptor.Kind == TevScriptTypeKindV3.Option)
        {
            ...
            if (variant == "None")
            {
                RequireValueKeys(value, context + ".$option", typeId, "None must not carry a payload", "type", "variant");
                return TevVariantV3.Empty(typeId, "None");
            }
            if (variant == "Some")
            {
                RequireValueKeys(value, context + ".$option", typeId, "Some requires exactly one payload", "type", "variant", "value");
        ...
        if (descriptor.Kind == TevScriptTypeKindV3.Result)
        {
            ...
            RequireValueKeys(value, context + ".$result", typeId, "Result requires exactly one payload", "type", "variant", "value");
    ...
    private static void Write(Utf8JsonWriter writer, string typeId, TevScriptValueV3 value, TevScriptTypeTableV3 table, string context, int depth)
    {
        ...
        if (descriptor.Kind == TevScriptTypeKindV3.Record)
        {
            if (value is not TevRecordV3 record || record.TypeId != typeId)
            {
                ValueFail(context, typeId, "expected exact record value");
                return;
            }
        ...
        if (value is not TevVariantV3 variant || variant.TypeId != typeId)
        {
            ValueFail(context, typeId, "expected exact variant value");
            return;
        }
    ...
    private static void RequireValueKeys(
        JsonElement value,
        string context,
        string typeId,
        string message,
    ...
    {
        if (value.ValueKind != JsonValueKind.Object) ValueFail(context, typeId, "expected object");
        ...
        if (!observed.SequenceEqual(target, StringComparer.Ordinal))
            ValueFail(context, typeId, message);
    }
