...
Gate Unity-3 treats Mono Player as a separate host/backend boundary. It explicitly selects `ScriptingImplementation.Mono2x` for `StandaloneWindows64`; the resulting artifact exposes the managed Core/Unity/harness assemblies and Mono runtime layout while rejecting `GameAssembly.dll`. The produced Player executes outside the Editor, reproduces the capability and numeric-boundary witnesses, and exits 0.

Gate Unity-4 treats IL2CPP/AOT as an independent backend boundary rather than inheriting the Mono result. Before building it reruns the exact certified Gate-3 commit in an isolated clone. The builder explicitly selects `ScriptingImplementation.IL2CPP` for `StandaloneWindows64`. The artifact must contain `GameAssembly.dll` and `il2cpp_data/Metadata/global-metadata.dat` while rejecting `MonoBleedingEdge`. The built Player must then execute the same TEV Core/capability/numeric-boundary witnesses and exit 0. The Gate-4 harness/build scripts are certification-only and do not modify the product Core or adapter.

Gate Unity-4 does **not** certify a physical Input System device or a real Animator Controller. Those remain provider-specific falsifiable boundaries. Browser execution and stable release remain separate project boundaries.
