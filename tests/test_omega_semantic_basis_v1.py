...
class OmegaFieldTests(unittest.TestCase):
    ...
    def test_tampered_field_hash_rejected(self) -> None:
        ...
        wire = {
            ...
            "facts": [
                {"relation": a.relation, "arguments": list(a.arguments), "fact_hash": a.fact_hash}
            ],
            "field_hash": "0" * 64,
        }
        with self.assertRaises(TevScriptError):
...
class OmegaTransformationTests(unittest.TestCase):
    ...
    def test_transformation_order_is_canonical(self) -> None:
        a = field_fact("world.a", (1,))
        b = field_fact("world.b", (2,))
        effects, resources = self._hashes()
        left = field_transformation(
            transformation_id="door.change",
            remove_fact_hashes=("f" * 64, "e" * 64),
            add_facts=(a, b),
    ...
    def test_tampered_transformation_hash_rejected(self) -> None:
        ...
        tx = field_transformation(
            transformation_id="ok", effect_set_hash=effects, resource_vector_hash=resources
        )
        wire = {
            ...
            "add_facts": [], "effect_set_hash": tx.effect_set_hash,
...
class OmegaApplyTests(unittest.TestCase):
    ...
    def test_unresolved_proof_requirements_are_not_pass(self) -> None:
        ...
        opened = field_fact("door.state", ("open",))
        tx = field_transformation(
            transformation_id="door.open.proved",
            remove_fact_hashes=(closed.fact_hash,),
            add_facts=(opened,),
            effect_set_hash=self.EFFECTS,
            resource_vector_hash=self.RESOURCES,
    ...
    def test_before_pin_mismatch_fails_closed(self) -> None:
        ...
        tx = field_transformation(
            transformation_id="door.badpin",
            required_before_hash="f"*64,
    ...
    def test_missing_remove_target_fails_closed(self) -> None:
        _closed, before = self._before()
        tx = field_transformation(
            transformation_id="door.missing",
            remove_fact_hashes=("e"*64,),
            effect_set_hash=self.EFFECTS, resource_vector_hash=self.RESOURCES,
        )
        with self.assertRaises(TevScriptError):
            apply_field_transformation(before, tx)
    ...
    def test_apply_receipt_tamper_is_rejected(self) -> None:
        ...
        wire = {
            ...
            "before_field_hash": receipt.before_field_hash,
            "transformation_hash": receipt.transformation_hash,
            "after_field_hash": receipt.after_field_hash,
            "effect_set_hash": receipt.effect_set_hash,
            "resource_vector_hash": receipt.resource_vector_hash,
            "proof_requirement_hashes": list(receipt.proof_requirement_hashes),
...
class OmegaSequenceTests(unittest.TestCase):
    ...
    def test_sequence_preserves_literal_order_and_exact_pins(self) -> None:
        closed = field_fact("door.state", ("closed",))
        opened = field_fact("door.state", ("open",))
        locked = field_fact("door.lock", (True,))
        ...
        second = field_transformation(
            transformation_id="door.lock",
            required_before_hash=middle.field_hash,
            add_facts=(locked,),
            effect_set_hash=self.EFFECTS, resource_vector_hash=self.RESOURCES,
        )
